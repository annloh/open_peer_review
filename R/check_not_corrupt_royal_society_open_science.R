...
article_id <- str_extract(list.files(path = "data\\royal_society_pdf_files\\open_science\\", pattern = "\\.pdf$"), "[0-9]+") #list all files in the folder and extract article ID
...
for (r in 1:length(article_id)){
  ...
  path <- paste("data\\royal_society_pdf_files\\open_science\\review", review_id, ".pdf", sep="") #used for open science
...
final_txt_file <- "data\\royal_society_pdf_files\\OS_pdf_list.txt" #used for open science
